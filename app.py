...
UPLOAD_BASE = "/var/data/uploads"    # Dossier fichiers persistants
...
def calendar_view():
    ...
    for r in rows:
        ...
        files_list = []
        if "files" in r.keys() and r["files"]:
            try:
    ...
    # Dates limites pour le récap
    last_day_number = pycal.monthrange(year, month)[1]
...
def check_reminders():
    """
    Vérifie les événements et envoie un email 24h avant la date (J+1).
    ...
    print(f"[REMINDER] {reminders_sent} rappel(s) envoyé(s).")
...
def api_check_reminders():
    """
    Endpoint appelé par ton cron externe (console.cron-job.org).

    Exemple d’URL :
    https://d3ntal-tech-calendrier-1.onrender.com/api/check-reminders?key=mySuperReminderKey2025
    """
